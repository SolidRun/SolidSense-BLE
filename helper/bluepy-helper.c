...
#include <assert.h>
#include <glib.h>
#include <time.h>   // addition L. Carré
...
static void try_open(void) {
	...
	time_t t;
    if (!fp) {
        fp = fopen ("/var/log/bluepy-helper.log", "w");
        if (! fp ) return;
        // Then we write a header
        t=time(NULL);
        lt=gmtime(&t);
        strftime(c_str,32,"%d/%m/%y-%H:%M:%S",lt);
        fprintf(fp,"Bluepy Helper New session %s",VERSION_STRING);
        fprintf(fp," ==== Start %s\n",c_str);
        fflush(fp);
        // assert(0); To test Python behavior when helper crash

    }
}

static void ss_btatt_debug(const char *str, void *user_data) {
		try_open();
		if (fp) {
			fprintf(fp,"BT ATT: %s\n",str);
			fflush(fp);
...
static void events_handler(const uint8_t *pdu, uint16_t len, gpointer user_data)
{
	...
	DBG("NOTIF handle=%d len=%d",handle,len);
...
// SolidSense add-on to trace low level messages
#ifdef BLUEPY_DEBUG_FILE_LOG
	bt_att_set_debug(g_attrib_get_att(attrib),ss_btatt_debug,NULL,NULL);
#endif
...
static void cmd_connect(int argcp, char **argvp)
{
	...
	// For test
	mtu=63;  // trying to force the MTU on connect send the requested MTU instead of 0
    iochannel = gatt_connect(opt_src, opt_dst, opt_dst_type, opt_sec_level,
                        opt_psm, mtu, connect_cb, &gerr);
